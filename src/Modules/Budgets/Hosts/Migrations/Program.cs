...
using SmartLedger.Modules.Budgets.Hosts.Migrations;
...
{
    var connectionString = context.Configuration.GetConnectionString("Budgets");

    services.AddDbContext<BudgetsWriteDbContext>(contextBuilder => contextBuilder.UseNpgsql(
        connectionString,
        optionsBuilder => optionsBuilder.MigrationsAssembly(Assembly.GetExecutingAssembly().FullName)));

    services.AddDbContext<BudgetsReadDbContext>(contextBuilder => contextBuilder.UseNpgsql(
        connectionString,
        optionsBuilder => optionsBuilder.MigrationsAssembly(Assembly.GetExecutingAssembly().FullName)));
